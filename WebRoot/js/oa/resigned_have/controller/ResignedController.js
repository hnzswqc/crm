...
Ext.define('Resigned.controller.ResignedController', {
    ...
    onResignedStateBtnClick: function(button, e, eOpts) {
        ...
        if(data.resMoneyType==DIC_RESIGNED_MONEY_TYPE_LJFF){
        	Ext.MessageBox.confirm("提示信息", "确定工资已经完结吗？", function (btnId) {
		        ...
		        Ext.create("Resigned.view.BalanceWindow",{"title":"工资结算","icon":"img/subsystem.png"}).show();
		        var gridPanel=Ext.getCmp("BalanceGridPanel");
		    	...
		    	gridPanel.store.load(function(records, operation, success) {  
		    		...
		    		Ext.each(records,function(record,i){
		    			if(record.data.monthState==DIC_RESIGNED_STATE_YES){
    ...
    onResignedGridPanelCellClick:function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts){
    	...
    	Ext.getCmp("ResignedStateBtn").enable(true);
    	Ext.getCmp("showBtn").enable(true);
    	Ext.getCmp("ResignedBackBtn").enable(true);
  ...
  //保存离职信息
   ...
   //关闭离职窗口
    ...
    reloadResignedGridPanelStore:function(data,text){
    	...
    	gridPanel.store.setProxy({
    		type: 'ajax',
            url: 'resigned/getResignedPage.do?resState='+DIC_RESIGNED_STATE_NO+'&userBankCard='+DIC_BANK_CARD_STATE_HAVE,
            extraParams:data,
            actionMethods: {
        		read: 'POST'
    		},
    ...
    //月份查询
    onMonthNumComboboxChange:function(field, newValue, oldValue, eOpts) {
    	var SearchForm = Ext.getCmp("ResignedSearchFromPanel");
    	this.reloadResignedGridPanelStore(SearchForm.getValues(),'高级查询-辞职员工信息');
    },
    //有无工资卡
    ...
    onResignedExcelBtnClick:function(button, e, eOpts) {
    	...
    	var panel = Ext.getCmp("ResignedSearchFromPanel");
        if(panel.isHidden()){
        	window.location.href=extBasePath+"resigned/getDownResignedList.do?resState="+DIC_RESIGNED_STATE_NO+"&userBankCard="+DIC_BANK_CARD_STATE_HAVE;
        }else{
        	var param = panel.getValues(true);
        	window.location.href=extBasePath+"resigned/getDownResignedList.do?resState="+DIC_RESIGNED_STATE_NO+"&userBankCard="+DIC_BANK_CARD_STATE_HAVE+"&"+param;  
        }
        Ext.Msg.hide();
    },
    //设置选择的月份
    ...
    //保存结算按钮
    onBalanceSaveBtnClick:function(button, e, eOpts) {
    	this.onUpdResignedMonth("确定要结算吗?",DIC_RESIGNED_STATE_YES);
    },
    //关闭结算按钮
    onBalanceCloseBtnClick:function(button, e, eOpts) {
    	Ext.getCmp("BalanceWindow").close();
    },
    //取消结算
    onBalanceDelBtnClick:function(button, e, eOpts) {
    	this.onUpdResignedMonth("确定要取消结算吗？",DIC_RESIGNED_STATE_NO);
    },
    //修改月份状态
    onUpdResignedMonth:function(showMsg,state){
    	var gridPanel = Ext.getCmp("BalanceGridPanel");
        var selectionModel = gridPanel.getSelectionModel();
        if(selectionModel.getCount()<=0){
       	 Ext.Msg.show({
        ...
        var monthUuid="";
        for ( var i = 0; i < selectionModel.getCount(); i++) {
			if(monthUuid!=""){
				monthUuid+=","+selectionModel.getSelection()[i].data.monthUuid;
			}else{
				monthUuid+=selectionModel.getSelection()[i].data.monthUuid;
			}
		}
        //获取人员信息
        var g = Ext.getCmp("ResignedGridPanel");
        var selectionModel = g.getSelectionModel();
        var data = selectionModel.getSelection()[0].data;
        
        Ext.MessageBox.confirm("提示信息", showMsg, function (btnId) {
 			if(btnId=='yes'){
	        Ext.Msg.wait('正在处理数据，请稍候...','提示');
	        Ext.Ajax.request({
	    		url:"resigned/updResignedMonth.do?monthState="+state,
	    		method:"POST",
	    		params:{"userUuid":data.userUuid,"monthUuids":monthUuid},
	    		success:function(responst){
	    			var result = JSON.parse(responst.responseText); 
	    			if(result.success){
	    				 Ext.Msg.hide();
    ...
    onResignedBackBtnClick: function(button, e, eOpts) {
        ...
        var data = selectionModel.getSelection()[0].data;
        Ext.MessageBox.confirm("提示信息", "确定要恢复正式员工吗？", function (btnId) {
            if (btnId == "yes") {
            	 Ext.Msg.wait('正在处理数据，请稍候...','提示');
                Ext.Ajax.request({
    ...
    init: function(application) {
        this.control({
            ...
            "combobox[id=monthNum]": {
                change: this.onMonthNumComboboxChange
            },
            ...
            "button[id=BalanceSaveBtn]": {
            	click: this.onBalanceSaveBtnClick
            },
            "button[id=BalanceCloseBtn]": {
            	click: this.onBalanceCloseBtnClick
            },
            "button[id=BalanceDelBtn]": {
            	click: this.onBalanceDelBtnClick
            },
