...
Ext.define('Leader.controller.LeaderController', {
    ...
    onBzAddBtnClick: function(button, e, eOpts) {
    	  Ext.create("Leader.view.MyWindow",{title:'添加班长工资',icon:'img/add.png'}).show();
    },
    //修改按钮
    onBzUpdBtnClick: function(button, e, eOpts) {
        ...
        Ext.create("Leader.view.MyWindow",{title:'修改班长工资',icon:'img/update.png'}).show();
    ...
    onBzExportBtnClick: function(button, e, eOpts) {
        ...
        if(panel.isHidden()){
        	window.location.href=extBasePath+"leader/getDownLeaderList.do";
        }else{
        	var param = panel.getValues(true);
        	window.location.href=extBasePath+"leader/getDownLeaderList.do?"+param;  
        }
        Ext.Msg.hide();
  ...
  //设置界面属性显示隐藏属性
    ...
    getLoggingTypeLeaderWages:function(){
        ...
        Ext.Ajax.request({
    		...
    		success:function(responst){
    			...
    			var result = JSON.parse(responst.responseText); 
    			if(result.success){
    				Ext.getCmp("loggingWages").setValue(result.data.loggingRealityWages);
    				Ext.getCmp("loggingRealityWages").setValue(result.data.loggingRealityWages);
    			}else{
    				 Ext.Msg.show({
           				title : '提示信息',
           				msg :'计算失败!请手动填写',
    ...
    onBzSearchBtnClick: function(button, e, eOpts) {
    	...
    	this.reloadBzGridPanelStore(searchForm.getValues(),'高级查询-班长工资');
    },
    //重置按钮
  ...
  //定时器,解决单击事件双击事件问题
   ...
   //用户双击事件
    ...
    onBzChoseUserBtnClick:function(button, e, eOpts) {
    	Ext.create("Leader.view.LoggingUserWindow",{"title":"双击选择员工","icon":"img/search.png"}).show();
    	
    },
    //选择产品按钮
    onBzChoseProductBtnClick:function(button, e, eOpts) {
    	Ext.create("Leader.view.ProductWindow",{"title":"双击选择产品","icon":"img/search.png"}).show();
    },
    //选择人员-刷新按钮
